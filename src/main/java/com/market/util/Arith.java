...
//    private static final int DEF_DIV_SCALE = 10;
    ...
    public static double sub(int v1, double v2) {
        BigDecimal b1 = new BigDecimal(Integer.toString(v1));
        BigDecimal b2 = new BigDecimal(Double.toString(v2));
        return b1.subtract(b2).doubleValue();
    }

    public static double sub(double v1, int v2) {
        BigDecimal b1 = new BigDecimal(Double.toString(v1));
        BigDecimal b2 = new BigDecimal(Integer.toString(v2));
        return b1.subtract(b2).doubleValue();
    }
     ...
     * 小数点以后10位，以后的数字四舍五入。
     *
     * 保留默认小数位数  该项目中为2位   具体保留位数视具体项目而定。
     *
     * @param v1 被除数
     * @param v2 除数
     * @return 两个参数的商
    ...
    public static double div(int v1, double v2) {
        return div(v1, v2, DEF_DIV_SCALE);
    }
    ...
    public static double div(int v1, double v2, int scale) {
        if (scale < 0) {
            throw new IllegalArgumentException(
                    "精度不可以小于0");
        }
